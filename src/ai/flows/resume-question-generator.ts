...
export type ResumeQuestionGeneratorInput = z.infer<typeof ResumeQuestionGeneratorInputSchema>;

const ResumeQuestionGeneratorOutputSchema = z.object({
  questions: z.array(z.string()).describe('An array of interview questions generated based on the resume.'),
});
export type ResumeQuestionGeneratorOutput = z.infer<typeof ResumeQuestionGeneratorOutputSchema>;

export async function resumeQuestionGenerator(input: ResumeQuestionGeneratorInput): Promise<ResumeQuestionGeneratorOutput> {
...
Generate 5-10 interview questions that are tailored to the candidate's resume. Focus on questions that assess the candidate's skills, experience, and suitability for potential roles.
